function [Acmm,hG,IG,vG,O_p_G] = Centroidal_Momentum_Matrix(RobotLinks,RobotParam,q,dq)
    ...
    %end of branches)
    for i = 1:NB
        RobotLinks(i).I_MAT_c = SpatialInertia(RobotLinks(i).I_MAT,RobotLinks(i).mass,RobotLinks(i).i_c_i);
        [i_X_pi(:,:,i),pi_X_i(:,:,i),Si(:,i),~,~] = jcalc(RobotLinks(i),q(i),0);
        pi_Xdual_i(:,:,i) = SpatialTransform_dual(pi_X_i(:,:,i));
        % tansformations from BASE
        if RobotLinks(i).Parent == 0
            O_X_i(:,:,i) = pi_X_i(:,:,i);
        else
            O_X_i(:,:,i) = O_X_i(:,:,RobotLinks(i).Parent)*pi_X_i(:,:,i);
        end
        %finding COM
        O_p_G = O_p_G + COMi(O_X_i(:,:,i),RobotLinks(i).i_c_i)*RobotLinks(i).mass;
    end
    O_p_G = O_p_G/sum([RobotLinks.mass]);
    %Add Inertia of Subtree to the parent body for it's subtree
    for i = flip(1:NB)
        if RobotLinks(i).Parent ~= 0  
            RobotLinks(RobotLinks(i).Parent).I_MAT_c = RobotLinks(RobotLinks(i).Parent).I_MAT_c + ...
    ...
    for i = 1:NB
        if RobotLinks(i).Parent == 0
            ...
            i_X_G(:,:,i) = i_X_pi(:,:,i)*i_X_G(:,:,RobotLinks(i).Parent);
        end
